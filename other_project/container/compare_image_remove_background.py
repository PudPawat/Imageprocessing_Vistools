...
# diff_img = img2
        ...
        right_contours = contour_center_dis(right_contours, (self.img2_process.shape[1] / 2, self.img2_process.shape[0] / 2), 50)
        ...
        if self.debug:
            ...
            print("right_contours of remove background",right_contours)
...
if __name__ == '__main__':
    ...
    folder = "F:\Pawat\Projects\Imageprocessing_Vistools\data\container\image\\200000_notch"
    ...
    if max(img1.shape) > 1000:
        ...
        img1 = orientation_detection.preprocess(img1,crop_circle)
    ...
    for j in range(len(names)):
        ...
        if max(img2.shape) > 1000: ## detect
            ## raw image input
            img2 = orientation_detection.preprocess(img2, crop_circle)
            cv2.imshow("raw", img2)
        else:
            if RANDOM:
