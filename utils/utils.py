...
def save_json(folder_dir, name_format, model_id, dict_to_save):
    with open(os.path.join(folder_dir,name_format+model_id+".json") , 'w') as openfile:
        ...
        *
        ...
        if self.__record_enable:
            ...
            record_formatter = logging.Formatter(
                "[%(show_time)s], [%(error_file)s], [%(error_func)s], [%(level_name)s], %(message)s")
